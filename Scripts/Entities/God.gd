...
func _physics_process(delta: float) -> void:
	...
	if health <= 0:
		var current_scene: Node = get_parent().get_parent().get_parent()
		var current_scene_path: String = current_scene.filename
		# Remove for death screen
		scene_controller.level0_variables["player_position"] = Vector2(216, 1312)
		scene_controller.change_scene(false, current_scene, current_scene_path, "res://Scenes/Levels/Level0.tscn")
